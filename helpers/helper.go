...
func (r RedisConnection) setKey(key, value string, expiry int) (string, error) {
	ctx := context.Background()
	SetResult := r.RedisClient.Set(ctx, key, value, time.Duration(expiry))
	_, err := SetResult.Result()
	if err != nil {
		return "failed", err
	} else {
		return "Key successfully set in redis", nil
	}
}
